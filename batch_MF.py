...
def matrix_factorization(ratings, u, v, epochs=150, alpha0=0.023, beta=0.045, suffix_name='batch'):
    y = np.zeros(epochs)

    nz_ratings = non_zero_matrix(ratings)
    bias = np.divide(np.sum(ratings), np.sum(nz_ratings.astype('float32'))).astype('float16')
...
def train(ratings, N, M, K, suffix_name='batch'):
    try:
        ...
        u_b = np.random.uniform(-0.05, 0.05, (N, K)).astype('float16')
        v_b = np.random.uniform(-0.05, 0.05, (M, K)).astype('float16')
        users = np.unique(np.nonzero(ratings)[0]).astype('int16')
        items = np.unique(np.nonzero(ratings[users, :])[1]).astype('int16')
